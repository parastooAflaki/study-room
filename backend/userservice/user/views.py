...
def requestresetpassword(request):
    ...
    if serializer.is_valid():
        ...
        responseSerializer = ResponseRequestResetPasswordSerilizer(data= serializer.data)
        if responseSerializer.is_valid():
            return Response(responseSerializer.data, status=status.HTTP_201_CREATED)
        else:
            return Response(responseSerializer.errors, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def resetpassword(request , token , format=None):
        ...
        if serializer.is_valid():
            ...
            if time_between_insertion.seconds / 60 > 15:
                ...
                user.save()
                responseSerializer = ResponseResetPasswordSerilizer(data = serializer.data)
                if responseSerializer.is_valid():
...
def search_users(request  , format=None ):
    ...
    print(searched_term)
    num_pages = User.objects.raw('SELECT COUNT(*) AS total , id AS id FROM user_user WHERE user_name LIKE %s' , [searched_term])
    found_users = User.objects.raw("SELECT * FROM user_user WHERE user_name LIKE %s ORDER BY id DESC LIMIT %s OFFSET %s " , [searched_term , limit_num ,skip_val ])

    response_serializer = FindedUsersSerializer(data = list(found_users), many = True)
    if response_serializer.is_valid():
        return HttpResponse( response_serializer.data
                , status = status.HTTP_200_OK)
    else:
        return HttpResponse( response_serializer.errors
